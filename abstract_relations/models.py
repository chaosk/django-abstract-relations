...
class AbstractManyToManyField(models.ManyToManyField):
    def __init__(self, to, **kwargs):
        """
        Retains `to` argument and keeps it in order to use it later during creation of new intermediary model.
        Raises an error when used without `through` argument, because in that case ManyToManyField should be used.
        """
        self._to = to

        if kwargs.get('through') is None:
            raise ImproperlyConfigured(
                "%s definition must include 'through' argument. Otherwise, resort to using %s" % (
    ...
    def contribute_to_class(self, cls, name, **kwargs):
        ...
        super(AbstractManyToManyField, self).contribute_to_class(cls, name, **kwargs)

    def _recreate_through_model(self, cls):
        """
        Overwrites the `through` attribute with newly created intermediary model. Makes the class available in same
        module, where the original intermediary model was defined.
        """
        if hasattr(self._to, '_meta'):  # Don't proceed during migrations
            self.remote_field.through = through = self._get_through_model(self.remote_field.through, cls, self._to)
    ...
    def _get_through_model(self, abstract_through, from_model, to_model):
        ...
        return type(abstract_through)(
            ...
            (abstract_through,),
            {
                ...
                'abstract_item': property(lambda this: getattr(this, from_model._meta.model_name)),
